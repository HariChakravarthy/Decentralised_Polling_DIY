...
const CONTRACT_ADDRESS = "0x90bB69fFd2EFd5c84F58Cc83fddFE718b154c2e8";

// Show time since poll was created
  ...
  const loadPolls = async (pollContract) => {
    try {
      ...
      for (let i = 0; i < count; i++) {
  	  ...
  	  const voted = await pollContract.hasUserVoted(account, i);  // Checking if the user has voted , added by hari 
	  console.log(`Poll ${i}`, poll);
	    ...
	    createdAt: poll.createdAt.toNumber() , // Added by Hari
	    hasVoted: voted,
            active: poll.active // Added by hari 
          });
        } catch (err) {
        ...
        {loading ? (
          ...
          <>
            ...
            <section className="polls">
		      ...
		      <p>Created {timeSince(poll.createdAt)}</p>
                      {/* added by hari */}
                      <div className="options">
                        ...
                        {poll.options && poll.options.map((option, idx) => (
                          <div key={idx} className="option">
                            ...
                            {!poll.hasVoted && (
                              <button onClick={() => vote(poll.id, idx)} disabled={poll.hasVoted}>Vote</button> // changed by hari 
                            )}
                          </div>
                        ))}
                      </div>
