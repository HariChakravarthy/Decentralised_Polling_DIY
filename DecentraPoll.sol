...
contract PollDApp {
    ...
    uint256 public MAX_POLLS_PER_USER = 1; // You can change the limit
    ...
    function createPoll(
        ...
        require(
            pollsCreated[msg.sender] < MAX_POLLS_PER_USER,
             "Poll limit reached for this user"
             );
    ...
    Poll storage poll = polls[_pollId];
    require(poll.creator == msg.sender, "Only creator can deactivate");
        ...
        )
    {
        ...
        Poll storage poll = polls[_pollId];
        // Check if the poll exists and is active
        require(poll.creator != address(0), "Poll does not exist"); //changed by hari safer check
        // require(bytes(poll.question).length > 0, "Poll does not exist"); // changed by hari 
